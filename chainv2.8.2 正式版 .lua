...
local CORRECT_KEY = "XK_83d0992ba16b0a979f7a50c79f8db6ae"
...
local function isStationElectrified(station)
	local powerStation = stasion:FindFirstChild("PowerStation")
...
local popupSuccess = pcall(function()
    WindUI:Popup({
        Title = "Chain XK",
        IconThemed = true,
        Content = " -支持无限子弹 电脑端T打开/关闭UI 脚本作者：美游",
        Buttons = {
            {
                Title = "关闭脚本",
                Variant = "Secondary",
...
local function triggerChainAlert(chain, skillName)
	...
	if cAlert.Dodge then
		...
		if myHRP and chainHRP then
			local dist = (myHRP.Position - chainHRP.Position).Magnitude
			-- 新增距离判断，超出半径不触发躲避
			if dist < cAlert.RingRadius then
